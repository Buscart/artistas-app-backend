...
import { posts, postMedia } from '../models/post.model.js';
...
export class PostService {
    ...
    static async getPostById(id) {
        ...
        const result = await db
            ...
            ...getPostFields(),
            authorId: users.id,
            authorName: sql `${users.firstName} || ' ' || COALESCE(${users.lastName}, '')`,
     ...
     */
    static async getAllPosts(limit = 10, offset = 0, type, userId, followingOnly, category) {
        const query = db
            .select({
            ...getPostFields(),
            authorId: users.id,
        ...
        const postsResult = rawPostsResult.map(rawPost => {
            const { authorId, authorName, authorAvatar, ...rest } = rawPost;
            return {
                ...rest,
                author: {
                    id: authorId,
     ...
     */
    static async updatePost(id, userId, updateData) {
     ...
     */
    static async addMediaToPost(postId, userId, mediaFiles) {
     ...
     * Comparte un post
     */
    static async sharePost(postId, userId, content) {
     ...
     */
    static async createComment(postId, userId, content, parentId, images, mentions, taggedArtists, taggedEvents, poll) {
