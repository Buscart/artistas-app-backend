...
export class RecommendationsService {
    ...
    async getSuggestedUsers(userId, limit = 10) {
        try {
            ...
            let currentArtistCategory = null;
            ...
            const sameCityUsers = currentUser.city
                ? await db
                    ...
                    followersCount: sql `(SELECT COUNT(*) FROM follows WHERE following_id = users.id)`,
                    mutualFollowers: sql `0`, // Se calculará después
                    score: sql `3`, // Score base para misma ciudad
                })
                    .from(users)
                    .where(and(eq(users.city, currentUser.city), not(inArray(users.id, excludeIds))))
                    .limit(5)
                : [];
            // ESTRATEGIA 2: Artistas de la misma categoría
            const sameCategoryArtists = currentArtistCategory && currentUser.userType === 'artist'
                ? await db
                    .select({
                    id: users.id,
                    firstName: users.firstName,
                    ...
                    isVerified: users.isVerified,
                    followersCount: sql `(SELECT COUNT(*) FROM follows WHERE following_id = users.id)`,
                    mutualFollowers: sql `0`,
                    score: sql `4`, // Score más alto para misma categoría
                })
                    .from(users)
                    .innerJoin(artists, eq(artists.userId, users.id))
                    .where(and(eq(artists.categoryId, currentArtistCategory), not(inArray(users.id, excludeIds))))
                    .limit(5)
                : [];
            ...
            const popularUsers = await db
                ...
                followersCount: sql `(SELECT COUNT(*) FROM follows WHERE following_id = users.id)`,
                mutualFollowers: sql `0`,
                score: sql `CASE WHEN users.is_verified THEN 2 ELSE 1 END`,
            })
                .from(users)
            ...
            for (const suggestion of allSuggestions) {
                if (!uniqueSuggestions.has(suggestion.id)) {
                    ...
                    const mutualFollowersResult = await db
                        ...
                        .from(follows)
                        .where(and(eq(follows.followerId, suggestion.id), inArray(follows.followingId, followingIds.length > 0 ? followingIds : [''])));
                    const mutualFollowers = mutualFollowersResult[0]?.count || 0;
    ...
    async getSimilarUsers(userId, limit = 10) {
        try {
            ...
            WHERE tag = ANY(${currentArtist.tags})
                ...
                .where(and(not(inArray(users.id, excludeIds)), sql `${artists.tags} && ${currentArtist.tags}` // Operador de intersección de arrays
            ))
                .orderBy(sql `(
          SELECT COUNT(*)
          FROM unnest(${artists.tags}) AS tag
