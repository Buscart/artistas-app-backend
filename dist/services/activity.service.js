...
export class ActivityService {
    ...
    async markActivitiesAsRead(userId, activityIds) {
        try {
            await db
                ...
                .set({ isRead: true })
                .where(and(eq(userActivities.userId, userId), inArray(userActivities.id, activityIds)));
            return { success: true };
        }
        catch (error) {
     ...
     */
    async recordProfileView(profileId, viewerId, viewerIp) {
        try {
     ...
     */
    async getProfileViewsCount(profileId, days = 30) {
        try {
            ...
            const result = await db
                .select({ count: count() })
                .from(profileViews)
                .where(and(eq(profileViews.profileId, profileId), gte(profileViews.createdAt, startDate)));
            return result[0]?.count || 0;
        }
        catch (error) {
     ...
     */
    async checkFollowerAchievements(userId) {
        try {
