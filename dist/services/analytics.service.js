...
export class AnalyticsService {
     ...
     */
    async getProfileViewsByDay(userId, days = 30) {
        try {
            const startDate = new Date();
            startDate.setDate(startDate.getDate() - days);
            const result = await db
                .select({
                date: sql `DATE(${profileViews.createdAt})`,
                views: sql `COUNT(*)`,
            })
                .from(profileViews)
                .where(and(eq(profileViews.profileId, userId), gte(profileViews.createdAt, startDate)))
     ...
     */
    async getFollowersGrowthByDay(userId, days = 30) {
        try {
            const startDate = new Date();
            startDate.setDate(startDate.getDate() - days);
            const result = await db
                ...
                newFollowers: sql `COUNT(*)`,
            })
                .from(follows)
                .where(and(eq(follows.followingId, userId), gte(follows.createdAt, startDate)))
     ...
     */
    async getEngagementStats(userId, days = 30) {
        try {
            const startDate = new Date();
            startDate.setDate(startDate.getDate() - days);
            const result = await db
                ...
                .from(userActivities)
                .where(and(eq(userActivities.userId, userId), gte(userActivities.createdAt, startDate)))
     ...
     */
    async getActivityDistribution(userId, days = 30) {
        try {
            const startDate = new Date();
            startDate.setDate(startDate.getDate() - days);
            const result = await db
