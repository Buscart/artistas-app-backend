...
export async function registerRoutes(app) {
    ...
    // Auth routes
    app.post('/api/hiring/requests', requireAuth, async (req, res) => {
        try {
    ...
    app.post('/api/users', async (req, res) => {
        try {
            const { uid, id, email, firstName, lastName, photoURL, profileImageUrl, userType, bio, city, isVerified } = req.body || {};
    ...
    app.put('/api/users/:id', async (req, res) => {
        try {
            const { id } = req.params;
            const { email, firstName, lastName, profileImageUrl, photoURL, userType, bio, city, isVerified } = req.body || {};
    ...
    app.post('/api/reviews', requireAuth, async (req, res) => {
        try {
            ...
            const canReview = await storage.reviews.canUserReview(userId, targetType, targetId);
            if (!canReview) {
    ...
    app.post('/api/hiring/responses', requireAuth, async (req, res) => {
        const user = req.user;
        try {
    ...
    function broadcastToCategory(categoryId, city, data) {
        if (!categoryId) {
