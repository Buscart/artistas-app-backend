...
export class DocumentsController {
    ...
    async uploadDocument(req, res) {
        try {
            ...
            const { data, error: uploadError } = await uploadFile('documents', filePath, file.buffer, { contentType: file.mimetype, upsert: false });
            if (uploadError) {
                throw new Error(`Error al subir archivo: ${uploadError.message}`);
            }
            const fileUrl = getPublicUrl('documents', data?.path || filePath);
    ...
    async updateDocumentStatus(req, res) {
        try {
            ...
            const document = await documentStorage.updateDocumentStatus(documentId, status, userId, rejectionReason);
            if (!document) {
                return res.status(404).json({ message: 'Documento no encontrado' });
            }
            return res.json(document);
