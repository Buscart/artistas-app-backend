...
async function getDisciplineIdByCode(code) {
    try {
        const result = await storage.db
...
async function getSpecializationIdByCode(code) {
    try {
        const result = await storage.db
    ...
    async completeOnboarding(req, res) {
        try {
            ...
            if (finalData?.firstName)
                updateData.firstName = finalData.firstName;
            if (finalData?.lastName)
                updateData.lastName = finalData.lastName;
            ...
            if (finalData?.shortBio)
                updateData.shortBio = finalData.shortBio;
            if (finalData?.interestedCategories)
                updateData.interestedCategories = finalData.interestedCategories;
            if (finalData?.interestedTags)
                updateData.interestedTags = finalData.interestedTags;
