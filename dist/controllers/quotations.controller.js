 ...
 */
export const getReceivedQuotations = async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ success: false, error: 'Usuario no autenticado' });
        ...
        if (status) {
            conditions.push(eq(userQuotations.status, status));
        }
...
export const respondToQuotation = async (req, res) => {
    try {
        ...
        const updated = await userContractsStorage.respondToQuotation(quotationId, data.quotedAmount, data.artistResponse);
        res.json({
            success: true,
            data: updated,
