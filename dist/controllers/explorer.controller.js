...
import { and, eq, gte, lte, or, sql, desc, ilike } from 'drizzle-orm';
import { users, artists, events, venues, services, artworks, highlightPhotos } from '../schema.js';
/**
...
class ExplorerController {
    ...
    static async getArtists(req, res) {
        try {
            const { query, city, category, tags, minPrice, maxPrice, discipline, // Subcategoría/disciplina del artista
            ...
            if (discipline && typeof discipline === 'string') {
                ...
                conditions.push(or(sql `LOWER(${artists.artistType}::text) LIKE LOWER(${disciplineTerm}::text)`, sql `${artists.tags}::text ILIKE ${disciplineTerm}`));
            }
            ...
            const artistsList = await db
                ...
                .orderBy(sortBy === 'price' ? artists.pricePerHour :
            ...
            let artistsWithPhotos;
            try {
                artistsWithPhotos = await Promise.all(artistsList.map(async (artist) => {
                    try {
    ...
    static async getEvents(req, res) {
        try {
            const { query, city, startDate, endDate, isFree, limit = '20', offset = '0', } = req.query;
            const conditions = [
    ...
    static async getServices(req, res) {
        try {
            ...
            const { query, category, minPrice, maxPrice, limit = '20', offset = '0', } = req.query;
            const conditions = [
    ...
    static async getArtworks(req, res) {
        try {
            ...
            const { query, category, city, minPrice, maxPrice, available, limit = '20', offset = '0', } = req.query;
            const conditions = [
    ...
    static async createArtwork(req, res) {
        try {
            ...
            const { name, description, category, images, price, dimensions, materials, year, available, stock, city, tags, showInExplorer, } = req.body;
            if (!name) {
    ...
    static async updateArtwork(req, res) {
        try {
            ...
            if (showInExplorer !== undefined)
                updateData.showInExplorer = showInExplorer;
            updateData.updatedAt = new Date();
    ...
    static async getAll(req, res) {
        try {
            ...
            const venuesConditions = [eq(venues.isAvailable, true)];
            if (city && typeof city === 'string') {
            ...
            const artworksConditions = [eq(artworks.showInExplorer, true)];
            if (city && typeof city === 'string') {
    ...
    static async searchEvents(req, res) {
        try {
            ...
            const results = await db
                ...
                .from(events)
                .where(or(ilike(events.title, `%${searchQuery}%`), ilike(events.description, `%${searchQuery}%`)))
