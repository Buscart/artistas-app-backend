 ...
 */
export const getMyHighlightPhotos = async (req, res) => {
    try {
 ...
 */
export const getUserHighlightPhotos = async (req, res) => {
    try {
 ...
 */
export const upsertHighlightPhoto = async (req, res) => {
    try {
        const userId = req.user?.id;
        ...
        const existingPhoto = await db
            .select()
            .from(highlightPhotos)
            .where(and(eq(highlightPhotos.userId, userId), eq(highlightPhotos.position, position)))
            .limit(1);
 ...
 */
export const updateHighlightPhotos = async (req, res) => {
    try {
        const userId = req.user?.id;
 ...
 */
export const deleteHighlightPhoto = async (req, res) => {
    try {
        const userId = req.user?.id;
        ...
        const [deletedPhoto] = await db
            ...
            .where(and(eq(highlightPhotos.userId, userId), eq(highlightPhotos.position, positionNum)))
            .returning();
        if (!deletedPhoto) {
 ...
 */
export const checkHighlightPhotosComplete = async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ error: 'No autorizado' });
        ...
        const missingPositions = [1, 2, 3, 4].filter(pos => !photos.some(p => p.position === pos));
        res.json({
            isComplete,
