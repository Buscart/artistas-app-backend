...
export const bookingsController = {
    ...
    async createBooking(req, res) {
        try {
            ...
            const { companyId, artistId, venueId, bookingType, title, description, startDate, endDate, setupTime, cleanupTime, roomName, expectedAttendees, services, equipment, contactName, contactEmail, contactPhone, basePrice, servicesPrice, totalPrice, deposit, clientNotes, } = req.body;
    ...
    async checkAvailability(req, res) {
        try {
            ...
            const conflictingBookings = await storage.db
                .select()
                .from(bookings)
                .where(and(eq(bookings.companyId, parseInt(companyId)), eq(bookings.status, 'confirmed'), 
            // Verificar solapamiento de fechas
            gte(bookings.endDate, start), lte(bookings.startDate, end)));
