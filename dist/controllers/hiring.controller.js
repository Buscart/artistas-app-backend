...
export const getAllHiringRequests = async (req, res) => {
    try {
        const { category, priceMin, priceMax, modality, limit = '50', offset = '0' } = req.query;
        let requests = await hiringStorage.getActiveHiringRequests();
        // Aplicar filtros
        if (category && typeof category === 'string') {
            requests = requests.filter(r => r.details?.toLowerCase().includes(category.toLowerCase()));
        }
        ...
        if (modality && typeof modality === 'string') {
            ...
            requests = requests.filter(r => modalities.some(m => r.details?.toLowerCase().includes(m.toLowerCase())));
        }
        // Paginación
 ...
 */
export const getHiringRequestById = async (req, res) => {
    try {
 ...
 */
export const respondToHiringRequest = async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ success: false, error: 'Usuario no autenticado' });
 ...
 */
export const getHiringResponses = async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ success: false, error: 'Usuario no autenticado' });
 ...
 */
export const acceptHiringResponse = async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ success: false, error: 'Usuario no autenticado' });
 ...
 */
export const rejectHiringResponse = async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ success: false, error: 'Usuario no autenticado' });
