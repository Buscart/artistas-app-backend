 ...
 */
export const createBulkTicketTypes = async (req, res) => {
    try {
        const eventId = parseInt(req.params.eventId);
        if (isNaN(eventId)) {
            return res.status(400).json({
        ...
        const ticketTypes = await ticketTypeStorage.createBulkTicketTypes(eventId, validation.data.map((tt) => ({ ...tt, price: String(tt.price) })));
        return res.status(201).json({
            success: true,
            data: ticketTypes,
...
export const updateTicketType = async (req, res) => {
    try {
        ...
        const existingTicketType = await ticketTypeStorage.getTicketTypeByEventId(ticketTypeId, eventId);
        ...
        const updatedTicketType = await ticketTypeStorage.updateTicketType(ticketTypeId, {
            ...validation.data,
