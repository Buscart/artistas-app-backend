...
export const getAllCollaborations = async (req, res) => {
    try {
        const { type, genre, limit = '50', offset = '0' } = req.query;
        let query = db
            .select({
        ...
        if (type && typeof type === 'string') {
            filteredResults = filteredResults.filter(r => r.collaboration.collaborationType === type);
        }
        if (genre && typeof genre === 'string') {
            filteredResults = filteredResults.filter(r => r.collaboration.genre?.toLowerCase().includes(genre.toLowerCase()));
        }
        // Paginación
