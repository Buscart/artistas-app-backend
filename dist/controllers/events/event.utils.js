...
export async function generateUniqueSlug(title, excludeId) {
    ...
    while (true) {
        const [existingSlug] = await db
            .select({ id: events.id })
            .from(events)
            .where(and(eq(events.slug, slug), excludeId ? ne(events.id, excludeId) : undefined))
            .limit(1);
        if (!existingSlug)
            break;
 ...
 */
export function formatEventResponse(event) {
    return {
        ...event,
        ...
        tags: Array.isArray(event.tags) ? event.tags : (typeof event.tags === 'string' ? event.tags.split(',').map((t) => t.trim()) : [])
    };
}
