import { and, eq, gte, lte, ne, or, sql, count, desc } from 'drizzle-orm';
...
export class EventService {
    ...
    static async createEvent(eventData, userId) {
        ...
        const newEvent = {
            ...
            shortDescription: eventData.bio || eventData.shortDescription || null,
            startDate: startDate,
    ...
    static async deleteEvent(eventId, userId) {
        ...
        const [attendeeCount] = await db
            .select({ count: count() })
            .from(eventAttendees)
            .where(and(eq(eventAttendees.eventId, eventId), or(eq(eventAttendees.status, 'approved'), eq(eventAttendees.status, 'checked_in'), sql `${eventAttendees.checkedInAt} IS NOT NULL`)));
    ...
    static async searchEvents(filters) {
        ...
        if (query && typeof query === 'string') {
            ...
            if (events.shortDescription) {
                searchConditions.push(sql `LOWER(${events.shortDescription}::text) LIKE LOWER(${searchTerm}::text)`);
            }
     ...
     */
    static async registerForEvent(eventId, userId, ticketTypeId) {
        // Verificar si el evento existe
        const [event] = await db
            .select()
            .from(events)
     ...
     */
    static async getEventAttendees(eventId, organizerId) {
        // Verificar ownership
        const [event] = await db
            .select()
            .from(events)
     ...
     */
    static async approveAttendee(eventId, attendeeId, organizerId) {
        // Verificar ownership
        const [event] = await db
            .select()
            .from(events)
     ...
     */
    static async rejectAttendee(eventId, attendeeId, organizerId) {
        // Verificar ownership
        const [event] = await db
            .select()
            .from(events)
     ...
     */
    static async moveToWaitlist(eventId, attendeeId, organizerId) {
        // Verificar ownership
        const [event] = await db
            .select()
            .from(events)
     ...
     */
    static async moveFromWaitlist(eventId, attendeeId, organizerId) {
        // Verificar ownership
        const [event] = await db
            .select()
            .from(events)
     ...
     */
    static async checkInAttendee(eventId, attendeeId, organizerId) {
     ...
     */
    static async undoCheckIn(eventId, attendeeId, organizerId) {
        // Verificar ownership del evento
        const [event] = await db
            .select()
            .from(events)
     ...
     */
    static async getAttendeeStats(eventId) {
        ...
        results.forEach((result) => {
            if (result.status in stats) {
                stats[result.status] = Number(result.count);
            }
        });
     ...
     */
    static async createReview(eventId, userId, data) {
     ...
     */
    static async respondToReview(eventId, reviewId, organizerId, response) {
     ...
     */
    static async getAttendedEvents(userId) {
        ...
        return cacheService.getOrSet(cacheKey, async () => {
            ...
            const attendedEvents = await db
                ...
                .leftJoin(users, eq(events.organizerId, users.id))
                .where(and(eq(eventAttendees.userId, userId), ne(events.organizerId, userId), // No incluir eventos que el usuario organizó
     ...
     */
    static async generateCertificate(eventId, odUserId) {
        // Verificar que el usuario asistió al evento
        const [attendance] = await db
            .select({
            id: eventAttendees.id,
            ...
            .from(eventAttendees)
            .where(and(eq(eventAttendees.eventId, eventId), eq(eventAttendees.userId, odUserId)));
        if (!attendance) {
            throw new Error('NOT_ATTENDEE');
        }
