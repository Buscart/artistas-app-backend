...
export const getUserOffers = async (req, res) => {
    try {
        ...
        const userOffers = await db
            .select()
            .from(offers)
            .where(or(eq(offers.clientId, userId), // ✅ clientId es string
...
export const getOfferById = async (req, res) => {
    try {
        ...
        const [offer] = await db
            .select()
            .from(offers)
            .where(and(eq(offers.id, parseInt(id)), or(eq(offers.clientId, userId), // ✅ clientId es string
        userId.startsWith('artist_')
            ? eq(offers.artistId, parseInt(userId.split('_')[1]))
            : sql `FALSE`)))
            .limit(1);
...
export const updateOfferStatus = async (req, res) => {
    try {
        ...
        const [offer] = await db
            .select()
            .from(offers)
            .where(and(eq(offers.id, parseInt(id)), or(eq(offers.clientId, userId), // ✅ string
        userId.startsWith('artist_')
            ? eq(offers.artistId, parseInt(userId.split('_')[1]))
            : sql `FALSE`)))
            .limit(1);
