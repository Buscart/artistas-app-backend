...
export class PostsController {
    ...
    static async getPosts(req, res, next) {
        try {
            ...
            const result = await PostService.getAllPosts(parseInt(limit), parseInt(offset), type, userId, followingOnly === 'true', category);
            res.json(result);
        }
        catch (error) {
    ...
    static async getUserPosts(req, res, next) {
        try {
            ...
            const posts = await PostService.getPostsByUser(userId, parseInt(limit), parseInt(offset));
            res.json({ data: posts });
        }
        catch (error) {
     ...
     */
    static async addMediaToPost(req, res, next) {
        try {
            const postId = parseInt(req.params.id);
            const userId = req.user?.id;
            if (isNaN(postId) || !userId) {
    ...
    static async createComment(req, res, next) {
        try {
            ...
            const comment = await PostService.createComment(postId, userId, content.trim(), parentId, images, mentions, taggedArtists, taggedEvents, poll);
