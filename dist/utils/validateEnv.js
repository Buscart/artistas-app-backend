...
export function validateEnv() {
    ...
    for (const envVar of envVariables) {
        ...
        if (envVar.required && !value) {
            errors.push(`❌ Variable requerida faltante: ${envVar.name}${envVar.errorMessage ? ` - ${envVar.errorMessage}` : ''}`);
            continue;
        }
        // Set default value if not provided
        if (!value && envVar.defaultValue) {
            process.env[envVar.name] = envVar.defaultValue;
            warnings.push(`⚠️  ${envVar.name} no definida, usando valor por defecto: ${envVar.defaultValue}`);
            continue;
        }
        // Validate value if validator is provided
        if (value && envVar.validator && !envVar.validator(value)) {
            errors.push(`❌ Valor inválido para ${envVar.name}${envVar.errorMessage ? ` - ${envVar.errorMessage}` : ''}`);
        }
    }
    // Log warnings
    ...
    if (errors.length > 0) {
        ...
        throw new Error('Validación de variables de entorno falló. Revisa los errores arriba.');
    }
