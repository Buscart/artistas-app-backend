...
const offerConnections = new Map();
const userConnections = new Map(); // NUEVO: Para notificaciones por usuario
wss.on('connection', async (ws, req) => {
    try {
        ...
        logger.warn('WebSocket autenticación falló', { error: e.message }, 'WS');
        ws.close(1008, 'Unauthorized');
        return;
    }
    ...
    ws.on('message', (message) => {
        try {
            ...
            logger.error('Error procesando mensaje WebSocket', error, 'WS');
        }
    });
    ws.on('close', () => {
...
app.broadcastResponse = async (offerId, response) => {
    ...
    if (connections) {
        try {
            ...
            logger.error('Error al obtener información del artista', error, 'WS');
        }
    }
};
app.broadcastStatusUpdate = async (offerId, responseId, status) => {
    const connections = offerConnections.get(offerId);
    if (connections) {
        try {
            ...
            logger.error('Error al enviar actualización de estado', error, 'WS');
        }
    }
};
// NUEVO: Función global para enviar notificaciones a usuarios específicos
app.notifyUser = (userId, notification) => {
