...
export const collectionsStorage = {
     ...
     */
    async getCollectionPosts(collectionId, userId) {
        ...
        const items = await db
            ...
            .where(and(eq(collectionItems.collectionId, collectionId), eq(collections.userId, userId)))
        ...
        const inspirationCounts = allPostIds.length > 0
            ...
            : [];
        const inspirationCountMap = new Map(inspirationCounts.map(ic => [ic.postId, ic.count]));
     ...
     */
    async removePostFromCollection(collectionId, postId, userId, postType = 'post') {
        // Verificar que la colección pertenezca al usuario
        ...
        const deleted = await db
            ...
            .where(and(eq(collectionItems.collectionId, collectionId), eq(collectionItems.postId, postId), eq(collectionItems.postType, postType)))
            .returning();
     ...
     */
    async isPostInCollections(userId, postId, postType = 'post') {
        ...
        const items = await db
            ...
            .from(collectionItems)
            .innerJoin(collections, eq(collectionItems.collectionId, collections.id))
            .where(and(eq(collectionItems.postId, postId), eq(collectionItems.postType, postType), eq(collections.userId, userId)));
        return items;
    },
    // ============================================================================
     ...
     */
    async getUserInspirations(userId, filters) {
     ...
     */
    async updateInspiration(id, userId, data) {
        const updateData = {
            updatedAt: new Date(),
        };
     ...
     */
    async removeInspiration(userId, postId, postType = 'post') {
        console.log('🗑️ removeInspiration storage - params:', { userId, postId, postType });
        try {
            const result = await db
                .delete(inspirations)
                .where(and(eq(inspirations.userId, userId), eq(inspirations.postId, postId), eq(inspirations.postType, postType)))
                .returning();
     ...
     */
    async isPostInspiration(userId, postId, postType = 'post') {
        const result = await db
            .select()
            .from(inspirations)
            .where(and(eq(inspirations.userId, userId), eq(inspirations.postId, postId), eq(inspirations.postType, postType)))
     ...
     */
    async getPostInspirationCount(postId) {
        const result = await db
            .select({
            count: sql `count(*)`,
        })
