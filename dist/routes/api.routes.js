...
router.get('/messages/threads', authMiddleware, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ message: 'No autenticado' });
...
router.post('/messages', authMiddleware, async (req, res) => {
    try {
        ...
        if (!receiverId || typeof receiverId !== 'string')
            return res.status(400).json({ message: 'receiverId requerido' });
...
v1.get('/profile/public/:id', profileController.getPublic); // Ruta pública
v1.get('/users/:userId', userController.getPublicProfile); // Ruta pública
...
v1.get('/featured', featuredController.getUserFeaturedItems);
v1.get('/users/:userId/featured', featuredController.getUserFeaturedItems);
v1.post('/featured', featuredController.createFeaturedItem);
...
protectedRoutes.put('/artist/me', (async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ message: 'No autenticado' });
        const { artistName, categoryId, 
        // Nueva estructura jerárquica
        disciplineId, roleId, specializationId, additionalTalents, // Array de discipline IDs (TADs)
        customStats, // Objeto { stat_key: value }
        // Campos legacy
        subcategories, tags, description, availability, isAvailable, stageName, gallery, pricePerHour, yearsOfExperience, portfolio, 
        // Información académica y profesional adicional
        education, languages, licenses, linkedAccounts, workExperience, 
        // Información profesional
        experience, artistType, travelAvailability, travelDistance, 
        // Precios
        hourlyRate, pricingType, priceRange, } = req.body || {};
        ...
        const safeAdditionalTalents = Array.isArray(additionalTalents)
            ? additionalTalents.filter((t) => typeof t === 'number')
            : undefined;
        ...
        if (!existing) {
            const created = await storage.createArtist({
                ...
                additionalTalents: artistPayload.additionalTalents,
                customStats: artistPayload.customStats,
                // Campos adicionales opcionales
                description: artistPayload.description,
                subcategories: artistPayload.subcategories,
                tags: artistPayload.tags,
                availability: artistPayload.availability,
                isAvailable: artistPayload.isAvailable,
                stageName: artistPayload.stageName,
                gallery: artistPayload.gallery,
                pricePerHour: artistPayload.pricePerHour,
...
v1.get('/dashboard/stats', authMiddleware, dashboardController.getStats.bind(dashboardController));
// Rutas de ofertas
v1.post('/offers', offerController.create);
v1.get('/offers', offerController.getAll);
v1.get('/offers/:id', offerController.getById);
v1.patch('/offers/:id/status', offerController.updateStatus);
// Rutas de perfiles
