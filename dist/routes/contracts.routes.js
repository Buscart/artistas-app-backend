...
router.post('/', authMiddleware, async (req, res) => {
    try {
        ...
        const { artistId, serviceId, serviceType, serviceName, description, amount, serviceDate, metadata, } = req.body;
...
router.put('/:id/status', authMiddleware, async (req, res) => {
    try {
        ...
        const updated = await userContractsStorage.updateContractStatus(parseInt(id), status, additionalData);
        res.json(updated);
    }
    catch (error) {
...
router.post('/:id/review', authMiddleware, async (req, res) => {
    try {
        ...
        const updated = await userContractsStorage.addReview(parseInt(id), parseInt(rating), review);
        res.json(updated);
    }
    catch (error) {
 ...
 */
router.get('/quotations/list', authMiddleware, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ error: 'Usuario no autenticado' });
...
router.post('/quotations', authMiddleware, async (req, res) => {
    try {
        ...
        const { artistId, serviceType, title, description, budgetMin, budgetMax, preferredDate, location, metadata, } = req.body;
 ...
 */
router.put('/quotations/:id/accept', authMiddleware, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ error: 'Usuario no autenticado' });
 ...
 * Rechazar cotización
 */
router.put('/quotations/:id/reject', authMiddleware, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ error: 'Usuario no autenticado' });
        ...
        const updated = await userContractsStorage.updateQuotationStatus(parseInt(id), 'rejected');
        res.json(updated);
    }
    catch (error) {
