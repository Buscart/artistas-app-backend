...
router.post('/', async (req, res) => {
    try {
        ...
        const [contract] = await db.select()
            .from(userContracts)
            .where(and(eq(userContracts.id, contractId), eq(userContracts.userId, userId)))
            .limit(1);
        if (!contract) {
...
router.get('/contract/:contractId/details', async (req, res) => {
    try {
        const userId = req.user?.uid;
        if (!userId) {
            return res.status(401).json({ success: false, message: 'No autorizado' });
...
function generateContractTerms(contract) {
