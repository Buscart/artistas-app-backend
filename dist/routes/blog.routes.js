...
blogRoutes.get('/', readUserRateLimit, asyncHandler(async (req, res) => {
    ...
    if (search) {
        conditions.push(or(like(blogPosts.title, `%${search}%`), like(blogPosts.content, `%${search}%`), sql `${blogPosts.tags}::text ILIKE ${`%${search}%`}`));
    }
...
blogRoutes.get('/user/:userId', readUserRateLimit, asyncHandler(async (req, res) => {
    ...
    const posts = await db
        ...
        .from(blogPosts)
        .leftJoin(users, eq(blogPosts.authorId, users.id))
        .where(and(eq(blogPosts.authorId, userId), eq(blogPosts.visibility, 'public')))
        .orderBy(desc(blogPosts.publishedAt));
    res.json({
        success: true,
        data: posts,
...
blogRoutes.post('/', authMiddleware, uploadRateLimit, asyncHandler(async (req, res) => {
    const userId = req.user.id;
    const { title, subtitle, content, category, subcategories, tags, featuredImage, gallery, videoUrl, visibility, allowComments, isFeatured, scheduledAt, } = req.body;
...
blogRoutes.patch('/:id', authMiddleware, moderateUserRateLimit, asyncHandler(async (req, res) => {
    ...
    const { title, subtitle, content, category, subcategories, tags, featuredImage, gallery, videoUrl, visibility, allowComments, isFeatured, } = req.body;
