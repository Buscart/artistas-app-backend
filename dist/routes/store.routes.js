...
storeRoutes.get('/me', authMiddleware, readUserRateLimit, asyncHandler(async (req, res) => {
    ...
    const userProducts = await db
        .select()
        .from(artworks)
        .where(and(eq(artworks.userId, userId), isNull(artworks.companyId)))
...
storeRoutes.get('/user/:userId', readUserRateLimit, asyncHandler(async (req, res) => {
    ...
    const userProducts = await db
        .select()
        .from(artworks)
        .where(and(eq(artworks.userId, userId), eq(artworks.available, true), eq(artworks.showInExplorer, true)))
        .orderBy(desc(artworks.createdAt));
    res.json({
        success: true,
        data: userProducts,
...
storeRoutes.post('/', authMiddleware, uploadRateLimit, asyncHandler(async (req, res) => {
    const userId = req.user.id;
    const { name, description, category, images, price, dimensions, materials, year, available, stock, tags, showInExplorer } = req.body;
    if (!name) {
...
storeRoutes.patch('/:id', authMiddleware, moderateUserRateLimit, asyncHandler(async (req, res) => {
    ...
    const [existing] = await db
        .select()
        .from(artworks)
        .where(and(eq(artworks.id, productId), eq(artworks.userId, userId)))
        .limit(1);
    if (!existing) {
        throw new AppError(404, 'Producto no encontrado', true, 'NOT_FOUND');
    }
