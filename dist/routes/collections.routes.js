...
router.get('/inspirations', authMiddleware, async (req, res) => {
    try {
        ...
        const filters = {};
        if (inspirationType)
            filters.inspirationType = inspirationType;
        if (tags)
 ...
 */
router.post('/inspirations', authMiddleware, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ error: 'User not authenticated' });
 ...
 */
router.put('/inspirations/:id', authMiddleware, async (req, res) => {
    try {
        const userId = req.user?.id;
        ...
        const inspiration = await collectionsStorage.updateInspiration(inspirationId, userId, { inspirationNote, tags, inspirationType });
        if (!inspiration) {
 ...
 */
router.delete('/inspirations/posts/:postId', authMiddleware, async (req, res) => {
    try {
        const userId = req.user?.id;
 ...
 */
router.get('/inspirations/posts/:postId/check', authMiddleware, async (req, res) => {
    try {
        const userId = req.user?.id;
        const postId = parseInt(req.params.postId);
        const postType = req.query.postType || 'post';
 ...
 */
router.get('/inspirations/posts/:postId/stats', async (req, res) => {
    try {
        const postId = parseInt(req.params.postId);
...
router.put('/:id', authMiddleware, async (req, res) => {
    try {
        ...
        const collection = await collectionsStorage.updateCollection(collectionId, userId, { name, description, isPublic, coverImageUrl });
        if (!collection) {
            return res.status(404).json({ error: 'Collection not found' });
        }
        res.json(collection);
