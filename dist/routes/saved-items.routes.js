...
import { readUserRateLimit, moderateUserRateLimit } from '../middleware/userRateLimit.middleware.js';
const router = Router();
/**
 * @swagger
 * /api/saved-items:
...
router.post('/', authMiddleware, moderateUserRateLimit, asyncHandler(async (req, res) => {
    ...
    const [existing] = await db
        .select()
        .from(savedItems)
        .where(and(eq(savedItems.userId, userId), eq(savedItems.postId, postId)))
        .limit(1);
    if (existing) {
